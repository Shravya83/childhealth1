...
if(isset($_POST['userlogin']))
{
  ...
  $sql= "SELECT * FROM `admin` where `emailid`='$user'";
  ...
  if($result->num_rows>0){
   ...
   $s = substr(str_shuffle(str_repeat("0123456789abcdefghijklmnopqrstuvwxyz", 5)), 0, 5);
         ...
         $sql1 = "UPDATE `admin` SET `temppass` = '$s' WHERE `emailid` = '$user';";
...
echo "<script type=\"text/javascript\">alert(\"OTP has been sent to your mail address\");</script>";
          ...
          echo"<META http-equiv=\"refresh\" content=\"0;adminnewpass?id=$user\">";
